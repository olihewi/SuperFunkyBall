...
VertexBuffer::VertexBuffer(Renderer& renderer, const std::vector<Vertex>& vertices) : stride(sizeof(Vertex))
{
	...
	bufferDesc.ByteWidth = UINT(sizeof(Vertex) * vertices.size()); // Size of the buffer in bytes
	bufferDesc.StructureByteStride = sizeof(Vertex); // Size of each vertex in bytes
