...
TriangleBuffer::TriangleBuffer(Renderer& renderer, const std::vector<unsigned int>& triangles)
{
	...
	bufferDesc.ByteWidth = static_cast<unsigned int>(triangles.size() * sizeof(unsigned int)); // Size of the buffer in bytes
	bufferDesc.StructureByteStride = sizeof(unsigned int); // Size of each index
...
void TriangleBuffer::Load(Renderer& renderer)
{
	renderer.GetContext()->IASetIndexBuffer(
		...
		DXGI_FORMAT_R32_UINT, // Format of the triangle data type (unsigned short)
